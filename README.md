...
* Design Overview.
### Image Rotation.
This service will rotate the input, raw image by a multiple of 90 degrees. 

stub.RotateImage is called which consequently creates the message type (NLImageRotateRequest) for it. The class generated by protoc, 'image_pb2.NLImageRotateRequest' contains rotation and image propoerties. As the server is fed the input file from the client, the image is required to be read as bytes in order to populate the data property of the image. The client then converts the bytes to a PIL.Image object which is saved as the output image file.
 The output image is then saved at the desired allocated path in the NLImage format. ** Note that due to the unary RPC used (limitations further discussed below), large images will be scaled down to 1000 pixels by 1000 pixels, while maintaining aspect ratio, in the client. This reduces the data size so that the entire image may be sent to the server as a single message. However, this means that the returned rotated image would also be scaled down to this size.
 
 ![Untitled](https://user-images.githubusercontent.com/62684338/169369744-57f24c2d-306c-4178-9671-1bf3dde77265.png)
